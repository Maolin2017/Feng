...
# COLNAME = 'DATASET_0409'
COLNAME = 'DATASET_0411_COLDONLY'
def job(COLNAME=COLNAME):
    ...
    colMeta = colMeta.query('{COLNAME}==1'.format(**locals()))
    ...
    # dfc = dfc.query('RUN_ID in ["191RQ","200R"] ')
    dfc = dfc.query('BASENAME=="read_counts.txt"')
    dfc = dfc.query("DATAACC in @colMeta.DATAACC")
    # dfc  = dfc.query('')
    checkShow(dfc)
    ...
    tab.set__colMeta(colMeta.set_index('DATAACC',drop=0))
