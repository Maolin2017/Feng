...
# pyutil.readBaseFile('/home/feng/work/results/')
targClu.to_csv('clu.csv')


gconf=  pyutil.readBaseFile('results/0219__prepareGCONF__Ath/gconf.npy').tolist()
cdsFile = gconf.GTF + '.cds.summit'
promFile = sutil.bed__leftSummit(cdsFile, 
                     opt='-s -l 1000 -r 1000',GSIZE=gconf.GSIZE,
                                inplace=0)
bed = sdio.extract_peak(promFile).set_index('acc',drop=0)
bed = bed.reindex(targClu.query('clu==1 & index in @bed.index').index)
# bed = bed.dropna()
pyutil.to_tsv(bed, 'chx-responsive-promoter.bed')
# pyutil.queryCopy(promFile,reader=sdio.extract_peak,)
