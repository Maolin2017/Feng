...
def worker(fname):
    ...
    sutil.to_tsv(dfc,'{BNAME}-all-fileterpeak-withdefline.csv'.format(**locals()),index=0)

    dfc = dfc.sort_values(['feat_acc','FC'],ascending=False).groupby('feat_acc').first()
    sutil.to_tsv(dfc,'{BNAME}-best-filterpeak-withdefline.csv'.format(**locals()),index=0)

map(worker,pyext.glob.glob('results/0501-poplar-peak2gene/output/*.tsv'))